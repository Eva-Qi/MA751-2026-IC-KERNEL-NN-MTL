...
sample size. Three independent attempts to improve the multilayer
perceptron (multi-seed averaging, prediction bagging, inner-CV
hyperparameter search) all fail to beat any single-seed linear baseline.
The Grinold-Kahn Barra factor model and Fama-MacBeth time-averaged
regression are the most robust point estimates (CPCV mean Sharpe~1.058
and~1.071, both with~$P(\text{Sharpe}<0)=0\%$). The audit cycle
documents two methodology pitfalls that generalize beyond this project:
mean-prediction bagging is incompatible with rank-based evaluation
metrics, and hyperparameter grid search actively degrades out-of-sample
performance when validation signal is below the noise floor. We also
retract two earlier ``failed'' results (Rung~3 GAM and Rung~3b
...
information coefficients (IC) in the 0.02--0.05 range; modern
...
A natural question follows: at the scale of a developed-market large-cap
universe, does adding model capacity---non-linearity, deep architectures,
multi-task learning---improve over a simple linear factor model?

Recent work answers \emph{yes} on broader universes. \citet{gu_kelly_xiu}
report that boosted-tree and neural-network models materially outperform
linear baselines on a $\sim$30,000-stock CRSP panel across six decades,
particularly on smaller-cap subsamples. Their finding has motivated
widespread industry adoption of ML-driven factor models.

This paper asks whether the Gu-Kelly-Xiu finding holds on a much smaller,
more tightly defined panel: 442 S\&P~500 large-caps across 119 monthly
cross-sections (2015--2024). At this scale---closer to the typical
buy-side equity research dataset---we evaluate twelve model variants
spanning a five-rung complexity ladder under a unified protocol:
identical features, identical walk-forward window, identical
long-short quintile portfolio construction, and identical rank
evaluation.
...
\item A reproducible, audit-graded comparison of twelve model variants
  on a frozen 119-month panel with 27 features.
\item Combinatorial Purged K-fold (CPCV) uncertainty bounds showing
  all top-six models' 5--95\% Sharpe distributions overlap.
\item Two new methodology lessons logged for general use:
...
\item Two documented \emph{retractions} of earlier ``failed'' models
  (GAM and XGBoost), where default hyperparameters were the source of
  failure rather than the methods themselves.
\end{itemize}
...
ranked by mean. Fig.~\ref{fig:cpcv-sharpe} shows the boxplot
visualization; Fig.~\ref{fig:pareto-bounds} plots the (IC, Sharpe)
Pareto frontier with one-standard-deviation error bars.
...
\subsection{The sample-size constraint as the binding factor}

Why do all top-six models look the same? The arithmetic is direct.
...
fall well within the noise floor. CPCV makes this concrete by showing
per-path Sharpe standard deviations of $\sigma\approx 0.5$--$0.8$ per
model---comparable to the cross-model differences in mean Sharpe.

\subsection{Why complexity does not help here}

Two complementary arguments:
\begin{itemize}[leftmargin=2em,topsep=2pt,itemsep=2pt]
\item \textbf{Structural}: our 27 features are predominantly
  factor-style aggregates with near-monotone relationships to forward
  returns. The GAM finding---that $n_{\text{splines}}=4$ (one internal
  knot, near-monotone shape) is optimal---supports this. Linear and
  lightly-curved models capture the bulk of the signal; additional
  capacity captures only noise.
\item \textbf{Statistical}: at IC$\approx$0.01 with our sample size,
  signal magnitude is below the noise floor of model selection. Even
  if a more complex model would win in expectation, the variance of
  estimating its win exceeds the win itself.
\end{itemize}

\subsection{Comparison to Gu-Kelly-Xiu (2020)}

\citet{gu_kelly_xiu} report substantial ML benefit on a much broader
universe ($\sim$30{,}000 stocks) over a longer history (1957--2016),
particularly on smaller-cap subsamples. Our null result on a 442-stock
S\&P~500 panel across 119~months is consistent with their finding,
not contradictory: the benefit of ML scales with sample size and with
the dispersion of stock characteristics. At large-cap S\&P~500 scale,
where stocks are well-followed and characteristics are tightly
distributed, the marginal value of ML is below the noise floor.

The natural threshold question is: at what scale does complexity start
to help? Our results bound it from below: $\sim$25{,}000 panel
observations is insufficient. Gu-Kelly-Xiu's results suggest
$\sim$10$\times$ more is sufficient. The gap between these is fertile
ground for follow-up research.

\subsection{Two methodology lessons (P7, P8)}

The audit cycle produced two general lessons that travel beyond this
project:

\begin{itemize}[leftmargin=2em,topsep=2pt,itemsep=2pt]
\item \textbf{P7: Match the ensemble operator to the evaluation metric.}
  Mean-prediction bagging is justified for Pearson/MSE evaluation; it
  is contraindicated for Spearman/rank evaluation. Practitioners using
  rank metrics should use rank-level averaging or the median prediction
  instead.
\item \textbf{P8: HP search hurts when validation signal is below the
  noise floor.} Before grid-searching, estimate the validation-noise SE
  via $\sim 1/\sqrt{n_{\text{val}}}$. If signal magnitude is comparable
  to this SE, prefer literature defaults or a small ($\le 5$-config)
  screen.
\end{itemize}

Both lessons are silently violated in casual ML practice and visible
...
\item \textbf{CPCV does not include Rung 4--5.} Multi-task and MoE
  models were dominated under walk-forward by every linear baseline.
  Adding them to CPCV would only further confirm dominance, not
  produce new findings; we omit them to preserve compute budget.
...
We construct and audit a five-rung complexity ladder for cross-sectional
monthly stock-return prediction on a 442-stock S\&P~500 panel
(2015--2024). Walk-forward backtests appear to support a Pareto
ranking with six top models clustered within annualized long-short
quintile Sharpe $\in[0.92, 1.13]$. \textbf{Combinatorial Purged K-fold
cross-validation reveals all top-six models' 5--95\% Sharpe bands
overlap}: Pareto comparisons are not statistically supported at our
sample size.
...
search) all fail to beat any single-seed linear baseline. The
Grinold-Kahn Barra factor model and Fama-MacBeth time-averaged
cross-sectional regression are the most robust point estimates, with
mean CPCV Sharpe~1.058 and~1.071 respectively, standard
deviations~0.544 and~0.494, and zero negative-Sharpe paths.

The audit cycle produced two methodology lessons that surface only
in low-SNR regimes: bagging operators must match the evaluation
metric's space (P7), and hyperparameter search degrades performance
when validation signal is below the noise floor (P8). We further
documented and retracted two earlier ``failed'' models (Rung~3 GAM
and Rung~3b XGBoost) where default hyperparameters---not the
methods---were the source of failure, and a code-bug ``failed'' model
(Rung~1d Barra) where a duplicate function definition was silently
overriding the correct implementation.

The thesis statement, made precise by CPCV: \emph{adding model
complexity does not improve cross-sectional return prediction in a
statistically meaningful way at our sample size}. With more data---a
longer historical window, a broader universe, or more frequent
cross-sections---this finding may be revisited, and indeed the larger
literature \citep{gu_kelly_xiu} suggests it would. At the scale of a
single decade of S\&P~500 large-caps, simple linear factor models,
properly specified, are not beaten.
