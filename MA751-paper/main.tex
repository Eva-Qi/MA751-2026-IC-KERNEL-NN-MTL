...
paths). The neural rungs do not beat the linear floor in this sample.
An audit cycle documents two methodology pitfalls that generalize
beyond this project: mean-prediction bagging is incompatible with
rank-based evaluation metrics, and hyperparameter grid search can
degrade out-of-sample performance when validation signal is below the
noise floor.
\end{abstract}
...
regime-informed MoE-MTL experiments are based on the earlier V2
14-feature panel and are disclosed separately in the results table.
...
\end{tabular}
\\[2pt]
{\footnotesize $^{\dagger}$Rung~5 plain MTL rows are computed on the
V2 14-feature panel and were not retuned for V3. Rung~6
Regime-Informed MoE-MTL and Enhanced MoE-MTL variants follow the same
dominated pattern and are excluded from the main table for brevity.
See Sec.~\ref{subsec:rung56-fail} for the neural and regime-gated
failure modes.}
\end{table}
...
The neural rungs do not beat the linear floor. The single-task MLP
(Rung~4) gives Sharpe~0.347 for seed~42, and the five-seed mean is
0.412. Plain MTL (Rung~5) improves over the single-task neural baseline
in its best configuration but still remains below the Ridge linear floor.
The regime-informed MoE-MTL extensions (Rung~6) add HMM-conditioned
expert specialization, but this additional structure does not rescue
out-of-sample performance in the 119-month panel.
...
% --------------------------------------------------------------
\subsection{Rung 5--6 Negative Transfer and Regime-Gated Failure Modes}
\label{subsec:rung56-fail}

The three Rung~5 plain MTL rows in Tab.~\ref{tab:wf-results} (5b, 5c,
5d) all fall below the linear floor: best is 5d at Sharpe~0.664, well
under even the worst Rung~1--2 model (1b IC-Ensemble at 0.844). Rung~6
Regime-Informed MoE-MTL adds HMM-conditioned expert specialization, but
the additional architecture does not rescue out-of-sample performance.
Three architecture-level failure modes explain why complexity does not
recover here.
...
\paragraph{Expert collapse in Rung 6.} The Regime-Informed and Enhanced
MoE-MTL variants use a softmax gate over $K{=}3$ expert MLPs, with the
...
ensemble of three weakly differentiated MLP heads.

\paragraph{HMM under-identification on early folds.} The 3-state HMM
gating signal is fitted per fold on five market-level features (VIX,
T10Y2Y, BAML high-yield spread, market return, and market realized
volatility) with full covariance emissions. Free parameters:
$K(K{-}1) = 6$ transition probabilities, $Kp = 15$ means, and
$K p(p{+}1)/2 = 45$ covariance entries, totaling ${\approx}66$. The
earliest walk-forward folds have only 60~training observations: the HMM
is materially under-identified. Posterior regime probabilities in those
folds revert toward weakly informative averages, contributing little
usable gating signal until $n_\text{train} \gtrsim 100$. By the time the
HMM is better-conditioned, expert collapse has already taken hold.
...
Rung~3. Rung~4--6 neural results are already dominated by linear
baselines under walk-forward and are omitted from CPCV for compute
budget reasons; this omission is listed as a limitation in
Sec.~\ref{sec:limitations}.
...
improved upon by any non-linear, deep, multi-task, or regime-gated
approach in our test window.
...
ranging from $\sim$27~(linear) to $\sim$3{,}000--4{,}000~(MLP) all
cluster within the same Sharpe band.
...
\begin{figure}[t]
  ...
  \caption{Effective degrees of freedom vs.\ walk-forward LS Sharpe.
    ...
    linear models to thousands for neural models), all top models
    cluster within Sharpe~$\in[0.83, 1.13]$. Additional parameters do
    not translate to additional predictive power at our sample size.}
  \label{fig:effective-df}
\end{figure}

The Rung~5--6 failure modes documented in Sec.~\ref{subsec:rung56-fail}
...
Importantly, this failure should not be interpreted as evidence that
MTL or MoE-MTL models are inherently inappropriate for return
prediction. These architectures are designed to exploit repeated
patterns across tasks, market states, and regimes. A longer panel,
higher-frequency decision grid, or broader stock universe would give
the shared encoder and regime gate more opportunities to observe
similar conditions multiple times. In that setting, the larger
Rung~5--6 models may become more competitive, and possibly superior,
because their additional parameters would be supported by more
independent training environments. Our result is therefore best read as
a sample-size limitation of the available monthly S\&P~500 panel, not a
general rejection of neural multi-task or regime-gated architectures.
...
\item \textbf{CPCV does not include Rung 4--6.} Walk-forward point
  ...
  (Regime-Informed MoE-MTL) appear either in Tab.~\ref{tab:wf-results}
  or in the audit outputs, but path-uncertainty bounds via CPCV are not
  computed for these neural rungs. Given their walk-forward
  underperformance relative to the linear floor, CPCV for Rung~4--6 was
  omitted for compute-budget reasons; this remains a limitation. Rung~5
  and Rung~6 results in particular use the V2 14-feature panel and were
  not retuned for V3.
...
\item \textbf{Hyperparameter search is not exhaustive.} GAM
  ...
  Rung~6 results may improve under better-tuned gating, but
  extrapolating from Rung~4--5 neural audits, the improvement is
  unlikely to exceed the linear floor in this sample.
...
top-seven models' 5--95\% Sharpe bands overlap; the Grinold-Kahn Barra
factor model and Fama-MacBeth regression are the most robust point
estimates (mean CPCV Sharpe~1.058 and~1.071, zero negative-Sharpe
paths). Three independent attempts to improve the single-task MLP fail to beat
any linear baseline, and neither plain MTL nor Regime-Informed MoE-MTL
rescues the neural rungs in this sample, though these architectures may
require longer, higher-frequency, or broader panels before their
additional capacity is estimable. The audit cycle surfaces two
methodology pitfalls---bagging-metric mismatch and hyperparameter
overtune on low-SNR validation---that generalize beyond this project.
