...
variants span a 5-rung complexity ladder: pooled linear, regularized
linear, smooth non-linear, single-task MLP, and multi-task with
...
We address this by constructing a \textbf{five-rung complexity ladder}
...
and multi-task mixture-of-experts. All variants are evaluated under a
...
% =====================================================================
\section{Methodology: Five-Rung Complexity Ladder}\label{sec:methodology}

We organize twelve model variants into five rungs of progressively
...
\begin{figure}[t]
  ...
  \caption{The five-rung complexity ladder. Each rung introduces a
    specific structural increase: regularization (1$\to$2),
    smooth non-linearity (2$\to$3), deep capacity with implicit
    interaction learning (3$\to$4), and shared representation across
    multiple targets (4$\to$5). Annotations show approximate parameter
...
\subsection{Rung 5 --- Multi-task and mixture-of-experts}
...
($n_\text{in}\to 64\to 32$) and per-task heads. Plus
...
(VIX, T10Y2Y, BAML high-yield spread). Plus \textbf{Enhanced MoE} with
...
under-identification). Regime-Gated MoE and Enhanced MoE follow the
same pattern and are excluded for brevity.}
\end{table}
...
\paragraph{Rung 4--5 dominated by linear.}
...
% --------------------------------------------------------------
\subsection{Rung 5 Negative Findings}\label{subsec:rung5-fail}

The three Rung~5 rows in Tab.~\ref{tab:wf-results} (5b, 5c, 5d) all
fall below the linear floor: best is 5d at Sharpe~0.664, well under
even the worst Rung~1--2 model (1b IC-Ensemble at 0.844). Three
architecture-level failure modes explain why complexity does not
recover here.
...
to the 8 top models from Rung~1, 2, and 3b. Rung 4--5 results are
...
The Rung~5 failure modes documented in Sec.~\ref{subsec:rung5-fail}
...
\item \textbf{Validation-split granularity.} The MLP/MTL training in
  Rung~4--5 uses a row-based ($n_\text{val}=\lfloor 0.1\cdot
...
\item \textbf{CPCV does not include Rung 4--5.} Walk-forward
  point estimates for Rung~4 (MLP) and Rung~5 (MTL/MoE) appear in
  Tab.~\ref{tab:wf-results}, but path-uncertainty bounds via CPCV are
  not computed. Walk-forward already places them well below the linear
  floor; CPCV would only further confirm dominance, not produce new
  findings, and we omit it to preserve compute budget. Rung~5 results
  in particular use the V2 14-feature panel (footnote in
  Tab.~\ref{tab:wf-results}); the architectures were not retuned for V3.
...
\item \textbf{Hyperparameter search is not exhaustive.} GAM
  ...
  $K{=}3$, $T{=}1$). \emph{Direction:} Rung~5 results may improve under
...
\begin{table}[h]
\centering
\caption{Hyperparameter inventory across the 5-rung ladder. ``Tested''
...
\midrule
5  & MoE                & $K{=}3$ experts, gate temp.~$T{=}1$ & no \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
  ...
  \caption{Hidden Markov Model (K=3 states) regime posteriors over the
    ...
    for the Rung~5 mixture-of-experts.}
