...
\midrule
5b~MTL (ret$+$ret\_3m)           & $-$0.0003 & $-$0.003 & 55.2 & 0.015 \\
5c~MTL (ret$+$vol)               & +0.0029 & 0.035 & 50.0 & 0.204 \\
5d~MTL (3-task)                  & $-$0.0087 & $-$0.080 & 50.0 & $-$0.227 \\
\bottomrule
\end{tabular}
\\[2pt]
{\footnotesize Rung~5 rows above are V3-retuned (same 27-feature panel
as Rungs~1--4). All three multi-task variants fall below the linear
floor; the 3-task variant 5d is negative on V3. See
Sec.~\ref{subsec:rung5-fail} for the failure mode discussion.
Rung~6 (Regime-Gated MoE, Enhanced MoE) follows the same dominated
pattern and is excluded for brevity.}
\end{table}
...
All three Rung~5 MTL rows (5b, 5c, 5d) fall below the linear floor on
the V3 panel. Best is 5c (ret$+$vol) at Sharpe~$+0.204$, still well
under even the worst Rung~1--2 model (1b IC-Ensemble at~0.844). The
3-task variant 5d, the most complex of the three, is
negative~($-0.227$): adding capacity \emph{hurts}.

\paragraph{Negative transfer and shared-encoder collapse.} The
Kendall-Gal-Cipolla uncertainty-weighted loss
\citep{kendall_gal_cipolla} normalizes per-task gradients by their
running variance, but cannot reverse gradient direction. When the
1-month and 3-month return gradients point in opposite directions in
encoder weight-space---a regime-shift phenomenon visible across the
2022--2023 inflation transition---the shared encoder representation
collapses toward whichever task gradient is louder per fold. On the V3
panel this manifests as 5d (3 tasks, the most opportunities for
gradient conflict) producing Sharpe~$-0.227$, materially worse than
5b~(0.015) and 5c~(0.204) which each carry only one auxiliary task.
Gradient-surgery methods such as PCGrad~\citep{yu_pcgrad} are
specifically designed for this case but were beyond our deadline scope.
...
\item \textbf{CPCV does not include Rung 4--6.} Walk-forward
  ...
  below the linear floor on the V3 panel; CPCV would only further
  confirm dominance, not produce new findings, and we omit it to
