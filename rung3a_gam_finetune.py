"""
Rung 3a GAM Fine-tune — n_splines fine-grained sweep [4, 6, 7, 8, 9].

Fills the gap between coarse sweep's n=3 and n=10.
Coarse result: n=5 Sharpe=0.79 >> n=10 Sharpe=0.55.
Question: is peak at 4, 5, 6, 7, 8, or 9?

Note: n=5 was already tested in coarse sweep — only [4, 6, 7, 8, 9] are new.
Sweep: n_splines ∈ [4, 6, 7, 8, 9]
Walk-forward: expanding window, min 60 months train, 1-month purge, 58 test months.
λ auto-tuned per fold via gridsearch (mirrors rung3a_gam_audit.py exactly).
"""
...
def main():
    print("=" * 80)
    print("RUNG 3A GAM FINE-TUNE — n_splines fine-grained sweep [4, 6, 7, 8, 9]")
    print("Coarse sweep: n=5 Sharpe=0.79 >> n=10 Sharpe=0.55")
    print("Goal: find true peak between n=3 and n=10")
    print("=" * 80, flush=True)
    ...
    # n=5 already tested in coarse sweep — test [4, 6, 7, 8, 9] to bracket peak
    n_splines_grid = [4, 6, 7, 8, 9]
    all_results = []
    ...
    out_csv = OUTPUT / "rung3a_n_splines_finetune.csv"
    results_df.to_csv(out_csv, index=False)
    print(f"\nSaved results -> {out_csv}", flush=True)

    # ── Final summary table ───────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("FINE-TUNE SUMMARY TABLE")
    print("=" * 80)
    cols = ["n_splines", "IC_mean", "IC_std", "IC_IR", "Hit_Rate", "LS_Sharpe", "n_months", "wall_sec"]
    print(results_df[cols].to_string(index=False))

    # ── Verdict vs coarse sweep reference ────────────────────────────────
    print("\n" + "=" * 80)
    print("VERDICT (vs coarse sweep reference: n=5 Sharpe=0.79, n=10 Sharpe=0.55)")
    print("=" * 80)
    ...
    if len(valid) == 0:
        ...
        best_row = valid.loc[valid["LS_Sharpe"].idxmax()]
        best_ic_row = valid.loc[valid["IC_mean"].idxmax()]

        print(f"  Best n_splines by LS_Sharpe: {int(best_row['n_splines'])} "
              f"(Sharpe={best_row['LS_Sharpe']:.3f}, IC={best_row['IC_mean']:+.4f})")
        print(f"  Best n_splines by IC_mean:   {int(best_ic_row['n_splines'])} "
              f"(IC={best_ic_row['IC_mean']:+.4f}, Sharpe={best_ic_row['LS_Sharpe']:.3f})")

        # Compare to coarse sweep n=5 reference
        COARSE_N5_SHARPE = 0.79
        COARSE_N5_IC = None  # IC not recorded in task description

        best_sharpe = float(best_row["LS_Sharpe"])
        delta = best_sharpe - COARSE_N5_SHARPE
        if delta > 0.02:
            print(f"\n  n={int(best_row['n_splines'])} BEATS coarse n=5 by ΔSharpe={delta:+.3f} — NEW OPTIMUM FOUND")
        elif abs(delta) <= 0.02:
            print(f"\n  n={int(best_row['n_splines'])} within noise of coarse n=5 "
                  f"(ΔSharpe={delta:+.3f}) — n=5 remains defensible optimum")
        else:
            print(f"\n  All fine-tune values below coarse n=5 Sharpe=0.79 — n=5 IS THE OPTIMUM")

        ic_vals = valid["IC_mean"].values
        ic_range = float(ic_vals.max() - ic_vals.min())
        print(f"\n  IC range across fine-tune configs: {ic_vals.min():+.4f} to "
              f"{ic_vals.max():+.4f} (Δ={ic_range:.4f})")

        sharpe_vals = valid["LS_Sharpe"].values
        sharpe_range = float(sharpe_vals.max() - sharpe_vals.min())
        print(f"  Sharpe range across fine-tune configs: {sharpe_vals.min():.3f} to "
              f"{sharpe_vals.max():.3f} (Δ={sharpe_range:.3f})")

        print(f"\n  RECOMMENDATION: Use n_splines=5 (coarse sweep winner) unless "
              f"fine-tune shows ΔSharpe > 0.05 for another value.")

    print("=" * 80)
    print(f"Outputs: {OUTPUT / 'rung3a_n_splines_finetune.csv'}")
    print(f"Log:     {OUTPUT / 'rung3a_n_splines_finetune_log.txt'}")
    print("=" * 80, flush=True)
